...
public class ScrapIndeed {
    private ArrayList<Job> _Scrap(String URL) {
        // INPUT: URL (String) url to request to the Indeed webpage, address includes job querying details
        // OUTPUT: (ArrayList<Job>) a list of Job object from the web scrapping based on job details
        // DESCRIPTION: workhouse to conduct the scrapping, details of the jobs to be queried from Indeed in encoded
        ...
        try {
            ...
            //for(Element div: divs) {
                ...
                if (rating != -1.0f) { job.setStarRating(rating); }
                if (payment != -1.0f) { job.setAnnualPay(payment); }
                jobs.add(job);
            }
            return jobs;

        } catch (IOException e) {
            // if query failed, return null to calling function indicating scrapping error
            return null;
        }
    }

    public ArrayList<Job> ScrapLocation(String location){
        // INPUT: location (String) job location in the form of a String
        // OUTPUT: (ArrayList<Job>) a list of Job object from the web scrapping based on job details
        // DESCRIPTION: encodes the job location to be queried into a web link and call helper function to scrap
        ...
        return _Scrap(search_link);
    }

    public ArrayList<Job> ScrapLocation(Location location){
        // INPUT: location (Location) job location in the form of a Location object
        // OUTPUT: (ArrayList<Job>) a list of Job object from the web scrapping based on job details
        // DESCRIPTION: encodes the job location to be queried into a web link and call helper function to scrap

        String location_str = "";
        if (location == null || (location.getCity() == null && location.getState() == null && location.getZipCode() == null)){
    ...
    public ArrayList<Job> Scrap(String que_not_location){
        // INPUT: que_not_location (String) job attribute (company, title...) other than location in the form of a String
        // OUTPUT: (ArrayList<Job>) a list of Job object from the web scrapping based on job details
        // DESCRIPTION: encodes the job attribute to be queried into a web link and call helper function to scrap
    ...
    public ArrayList<Job> Scrap(String que_not_location, String location){
        // INPUT: que_not_location (String) job attribute (company, title...) other than location in the form of a String
        // INPUT: location (String) job location in the form of a String
        // OUTPUT: (ArrayList<Job>) a list of Job object from the web scrapping based on job details
        // DESCRIPTION: encodes the job attributs to be queried into a web link and call helper function to scrap
        //   Indeed only supports location as a separate query input, the reset of the attributes are all grouped
        //   together into one query input
        String search_link = "https://www.indeed.com/jobs?q=";
        if (que_not_location == null || que_not_location.equalsIgnoreCase("")) {
