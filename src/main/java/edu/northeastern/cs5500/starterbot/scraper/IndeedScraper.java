...
public class IndeedScraper implements Scraper {
    ...
    private float _scapeRating(Element div) {
        // set company rating, Indeed does not post rating for specific job on scrap page, only
        // rating
    ...
    private float _scrapeAnnualPayment(Element div) {
        float payment = 0.0f;
        try {
            // due to definition of the AnnualPayment variable, only load jobs with exact annual
            // payment info
