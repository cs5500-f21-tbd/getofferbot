...
public class SearchCommand implements Command {
    ...
    public void onSlashCommand(SlashCommandEvent event) {
        ...
        String content = "JobId     Company     Level   Location\n" +
    ...
    public CommandData getCommandData() {
        //TODO: either make them global static, or dynamically refresh such choices based on data from DB.
        OptionData companyOptions = new OptionData(
                                        OptionType.STRING,
                                        "company",
                                        "What companies do you want to search for?");
        for (String choice : Arrays.asList("Amazon", "Google", "Meta")) {
            companyOptions.addChoice(choice, choice);
        }

        OptionData levelOptions = new OptionData(
                                        OptionType.STRING,
                                        "level",
                                        "What levels do you want to search?");
        for (String choice : Arrays.asList("Intern", "New Grad", "Senior")) {
            levelOptions.addChoice(choice, choice);
        }

        OptionData locationOptions = new OptionData(
                                        OptionType.STRING,
                                        "location",
                                        "What locations do you want to search?");
        for (String choice : Arrays.asList("Seattle", "Silicon Valley", "New York")) {
            locationOptions.addChoice(choice, choice);
        }

        OptionData wfhOptions = new OptionData(
                                        OptionType.STRING,
                                        "wfh",
                                        "What wfh do you want to search?");
        for (String choice : Arrays.asList("Yes", "No")) {
            wfhOptions.addChoice(choice, choice);
        }

        OptionData sponsorshipOptions = new OptionData(
                                        OptionType.STRING,
                                        "sponsorship",
        ...
        OptionData sortOptions = new OptionData(
                                        OptionType.STRING,
                                        "sort",
        ...
        return new CommandData(this.getName(), "Search for jobs.")
                .addOptions(companyOptions, levelOptions, locationOptions, wfhOptions, sponsorshipOptions, sortOptions);
    }
}
